...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/ai-tutor/chat', async (req, res) => {
    try {
      ...
      const responses = {
        'что такое машинное обучение': `Машинное обучение - это способ научить компьютер находить закономерности в данных и делать предсказания.

Простая аналогия: представьте, что вы учите ребенка распознавать животных. Вы показываете ему много фотографий кошек и говорите "это кошка", показываете собак и говорите "это собака". Через некоторое время ребенок научится различать их сам.

Машинное обучение работает похоже - мы показываем компьютеру много примеров, а он учится находить различия и сходства.`,

        'как работают нейронные сети': `Нейронные сети вдохновлены работой человеческого мозга, но намного проще.

Представьте сеть друзей в соцсети:
- Каждый человек (нейрон) получает информацию от друзей
- Обрабатывает её и передает дальше
- Связи между людьми имеют разную "силу" (веса)

В нейросети информация течет от входа к выходу через множество простых вычислений. Чем больше слоев - тем сложнее паттерны может найти сеть.`,

        'что такое искусственный интеллект': `ИИ - это технология, которая позволяет компьютерам выполнять задачи, требующие "ума": понимать речь, распознавать картинки, принимать решения.

Примеры ИИ в вашей жизни:
- Рекомендации YouTube и Netflix
- Автокоррекция в телефоне  
- Навигация в картах
- Голосовые ассистенты

ИИ не "думает" как человек, но может решать похожие задачи быстро и точно.`
      };
      ...
      for (const [key, value] of Object.entries(responses)) {
        if (messageText.includes(key)) {
          response = {
            success: true,
            message: value,
            suggestions: ["Хотите узнать больше?", "Есть вопросы по теме?"],
            relatedTopics: ["Алгоритмы", "Данные", "Практика"]
          };
          break;
        }
      }
      ...
      if (!response) {
        response = {
          success: true,
          message: `Интересный вопрос! Я помогу разобраться с этой темой.

Попробуйте переформулировать вопрос или спросите о:
- Основах машинного обучения
- Принципах работы нейронных сетей  
- Применении ИИ в реальной жизни
- Практических примерах

Что именно вас больше всего интересует?`,
          suggestions: ["Основы ML", "Нейронные сети", "Примеры ИИ"],
          relatedTopics: ["Алгоритмы", "Данные", "Практика"]
        };
      }
  ...
  app.get('/api/ai-tutor/suggestions', async (req, res) => {
    try {
      const suggestions = [
